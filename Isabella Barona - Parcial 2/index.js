...
io.on('connection', (socket) => {
    ...
    socket.on('orderForArduino', (orderForArduino) => {
        serialPort.write(orderForArduino);
...
expressApp.post('/score', (request, response) => {
    ...
    socket.emit('orderForArduino', Order);
})
...
expressApp.post('/game-over', (request, response) => {
    ...
    let gameOver = request.body.gameOver;

    console.log('Game over');

    socket.emit('orderForArduino', gameOver);
})
