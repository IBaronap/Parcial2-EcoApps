...
socket.on('arduinoMessage', (arduinoMessage) => { //Recibe mensaje arduino
    ...
    let { actuatorValue, btnAValue, btnBValue } = arduinoMessage;

    moveX(actuatorValue);
    PlayerWidth(btnAValue);
    BallSpeed(btnBValue);
});

function moveX(actuatorValue){
    player.x = map(actuatorValue, 0, 1023, 0, windowWidth);
}

function PlayerWidth(btnAValue){
    if(btnAValue == 1){
        player.width -= 10;
    }
}

function BallSpeed(btnBValue){
    if(btnBValue == 1){
...
async function sendScore(point) {
    ...
    const data = {
        ...
        body: JSON.stringify({Point: point, Order: 'S'}),
    };
...
async function gameResult() {
    ...
    const dataGame = {
        ...
        body: JSON.stringify({Order: 'L'}),
    };

    await fetch('/game-over', dataGame);
